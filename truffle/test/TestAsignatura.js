...
contract("Uso normal de Asignatura:", (accounts) => {
  ...
  const evaluaciones = [
    {
      nombre: "Parcial 1",
      fecha: Date.now() + 60 * 24 * 3600000,
      porcentaje: 30,
      minimo: 500,
    },
    {
      nombre: "Parcial 2",
      fecha: Date.now() + 120 * 24 * 3600000,
      porcentaje: 30,
      minimo: 500,
    },
    {
      nombre: "Practica 1",
      fecha: Date.now() + 50 * 24 * 3600000,
      porcentaje: 20,
      minimo: 400,
    },
    {
      nombre: "Practica 1",
      fecha: Date.now() + 110 * 24 * 3600000,
      porcentaje: 20,
      minimo: 400,
    },
  ];
  ...
  const calificaciones = [
    ...
    [
      ...
      [2, 750],
      [2, 800],
    ], // Notas del segundo alumno
    [
      [0, 0],
      [0, 0],
      [0, 0],
      [0, 0],
    ], // Notas del tercer alumno
    [
      [2, 950],
      [2, 800],
      [1, 0],
      [1, 0],
    ], // Notas del cuarto alumno
    [
      [1, 0],
      [1, 0],
      [1, 0],
      [1, 0],
    ], // Notas del quito alumno
  ];
  ...
  it("Solo el owner puede asignar el coordinador.", async () => {
    ...
    try {
      await asignatura.setCoordinador(desconocido, "nombre", {
        from: desconocido,
      });
  ...
  it("A̱adir los profesores.", async () => {
    ...
    assert.equal(
      pl,
      profesores.length,
      "No se han a̱adido todos los profesores.",
    );
    ...
    for (const profesor of profesores) {
      ...
      assert.equal(
        p,
        profesor.nombre,
        "El nombre del profesor no se ha guardado bien.",
      );
    }
  });
  ...
  it("Solo el owner puede a̱adir profesores.", async () => {
    ...
    try {
      await asignatura.addProfesor(desconocido, "nombre", {
        from: desconocido,
      });
  ...
  it("Automatricularse.", async () => {
    ...
    for (const alumno of alumnos1) {
      await asignatura.automatricula(alumno.nombre, alumno.dni, alumno.email, {
        from: alumno.addr,
      });
    }
    ...
    for (const alumno of alumnos1) {
      ...
      assert.equal(
        a[0],
        alumno.nombre,
        "El nombre de un alumno no se ha guardado bien.",
      );
      assert.equal(
        a[1],
        alumno.dni,
        "El DNI de un alumno no se ha guardado bien.",
      );
      assert.equal(
        a[2],
        alumno.email,
        "El email de un alumno no se ha guardado bien.",
      );
    }
  ...
  it("El owner puede matricular alumnos.", async () => {
    ...
    try {
      ...
      await asignatura.matricular(alumno.nombre, alumno.dni, alumno.email, {
        from: owner,
      });
      assert.fail("No se puede matricular un alumno dos veces.");
    } catch {}
  ...
  it("El DNI de los alumnos no puede repetirse.", async () => {
    ...
    try {
      await asignatura.automatricula("nombre", alumnos[0].dni, "email", {
        from: desconocido,
      });
  ...
  it("No puede dejarse el nombre ni el dni vacio al automatricularse.", async () => {
    ...
    try {
      await asignatura.automatricula("", "1111", "email", {
        from: desconocido,
      });
    ...
    try {
      await asignatura.automatricula("nombre", "", "email", {
        from: desconocido,
      });
  ...
  it("Crear las evaluaciones.", async () => {
    ...
    for (const evaluacion of evaluaciones) {
      await asignatura.creaEvaluacion(
        evaluacion.nombre,
        evaluacion.fecha,
    ...
    assert.equal(
      el,
      evaluaciones.length,
      "No se han a̱adido todas las evaluaciones.",
    );
    ...
    for (let i = 0; i < evaluaciones.length; i++) {
      ...
      assert.equal(
        ...
        "El nombre de una evaluacion alumno no se ha guardado bien.",
      );
      assert.equal(
        e[1],
        evaluaciones[i].fecha,
        "La fecha de una evaluacion no se ha guardado bien.",
      );
      assert.equal(
        e[2],
        evaluaciones[i].porcentaje,
        "El porcentaje de una evaluacion no se ha guardado bien.",
      );
      assert.equal(
        e[3],
        evaluaciones[i].minimo,
        "Los puntos minimos de una evaluacion no se han guardado bien.",
      );
    }
  });
  ...
  it("Crear las calificaciones.", async () => {
    ...
    for (let ai = 0; ai < calificaciones.length; ai++) {
      ...
      for (let ei = 0; ei < evaluaciones.length; ei++) {
        await asignatura.califica(
          alumnoAddr,
          ei,
          calificaciones[ai][ei][0],
          calificaciones[ai][ei][1],
          { from: profesores[0].addr },
        );
      }
    }
    ...
    for (let ai = 0; ai < calificaciones.length; ai++) {
      ...
      for (let ei = 0; ei < evaluaciones.length; ei++) {
        ...
        assert.equal(
          c[0].toNumber(),
          calificaciones[ai][ei][0],
          "El tipo de una calificacion no se ha guardado bien.",
        );
        assert.equal(
          c[1].toNumber(),
          calificaciones[ai][ei][1],
          "La nota de una calificacion no se ha guardado bien.",
        );
      }
    }
  });
  ...
  it("Solo puede calificar un profesor.", async () => {
    ...
    try {
      await asignatura.califica(alumnos[0].addr, 0, 0, 0, {
        from: desconocido,
      });
  ...
  it("Consultar mi nota.", async () => {
    ...
    for (let i = 0; i < evaluaciones.length; i++) {
      ...
      assert.equal(
        c1[0].toNumber(),
        calificaciones[1][i][0],
        "El tipo de una calificacion mia no se ha guardado bien.",
      );
      assert.equal(
        c1[1].toNumber(),
        calificaciones[1][i][1],
        "La nota de una calificacion mia no se ha guardado bien.",
      );
    }
  });
  ...
  it("Consultar mi nota final.", async () => {
    ...
    for (let i = 0; i < alumnos.length; i++) {
      ...
      assert.equal(
        nf[0].toNumber(),
        notasFinales[i][0],
        "El tipo de una calificacion final no se ha calculado bien.",
      );
      assert.equal(
        nf[1].toNumber(),
        notasFinales[i][1],
        "La nota de una calificacion final no se ha calculado bien.",
      );
    }
  });
